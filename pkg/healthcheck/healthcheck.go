...
func (hc *HealthChecker) checkRemoteClusterConnectivity() error {
	...
	for _, s := range secrets.Items {
		secret := s
		config, clusterName, _, err := sm.ExtractRemoteClusterAPICredentials(&secret)
		if err != nil {
		...
		if err != nil {
			errors = append(errors, fmt.Sprintf("* secret: [%s/%s] cluster: [%s]: unable to parse api config: %s", secret.Namespace, secret.Name, clusterName, err))
			continue
		}

		remoteAPI, err := k8s.NewAPIForConfig(clientConfig, "", []string{}, requestTimeout)
		if err != nil {
			errors = append(errors, fmt.Sprintf("* secret: [%s/%s] cluster: [%s]: could not instantiate remote api: %s", secret.Namespace, secret.Name, clusterName, err))
			continue
		}
		...
		if !canGet || !canList || !canWatch {
			errors = append(errors, fmt.Sprintf("* cluster: [%s]: Insufficient permissions GET: %v, LIST: %v, WATCH: %v", clusterName, canGet, canList, canWatch))
		}
	}
